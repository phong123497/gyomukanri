...
// SystemName: 業務管理
// VueName: NendoInput(仮作成版)
//
// <<Modification History>>
// Version  | Date       | Updated By           | Content
// ---------+------------+----------------------+---------------------------------
// 01.00.00 | 2023/08/31 |   AMC                | 新規作成
//================================================================================
 -->
<template>
	<div
		class="date-input"
		:style="switchDisplayMode"
	>
		<input
			type="text"
			autocomplete="off"
		...
		<v-date-picker
			...
			ref="calendar"
			@popoverDidShow="handleShow"
		>
			<template v-slot="{ inputValue, togglePopover }">
				<button
					v-on:click="togglePopover({placement: 'bottom-end', visibility: 'hover-focus' })"
					tabindex="-1"
					v-bind:disabled="disabledFlg"
				>
	...
	export default {
		...
		data() {
			return {
				...
				isError: false,											// エラー判定
				dateBlurFormat: 'YYYY',									// 表示フォーマット(ロストフォーカス時)
				dateFocusFormat: 'YYYY',								// 表示フォーマット(フォーカス時)
