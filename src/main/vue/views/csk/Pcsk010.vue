	...
	<div>
		...
		<main id="PCSK010">
			<div class="contents__block--exist_return">
				...
				<!-- タイトル -->
				<h1 class="header__block-title">CSV取込</h1>
								...
								<td>
									<div class="contents__block-radio">
										<list-box
											...
											v-bind:listItems="disp.listButtonListItems"
											v-bind:name="'取込対象'"
											v-bind:requireFlg="true"
											v-bind:tabindex="'1'"
											v-model="form.torikmTarget"
										/>
									</div>
								</td>
							</tr>
							<tr class = "border-b" v-if="form.torikmTarget == 'S'">
								<td>
									諸伝票
								</td>
								<td>
									<div class="contents__block-radio">
										<radio-button
											...
											v-bind:name="'諸伝票'"
											v-bind:radioGroupName="'varDempyo'"
											v-bind:requireFlg="form.torikmTarget == 'S'"
											v-bind:tabindex="'2'"
											v-model="form.varDempyo"
										/>
									</div>
								</td>
							</tr>
						...
						<section>
							<div class="contents__block-top">
								...
								<div class="contents__block-top__area--float_r" style="padding-top:10px">
									<span class="margin-r-780px">{{tableListCount}}件</span>
								</div>
							</div>
...
export default {
	...
	created: function() {
		...
		// 諸伝票-Coreを選択する
		this.form.varDempyo = "0";
	},
	mounted: async function() {
		// ** フォーカス設定 **
		await this.$nextTick();
		this.$refs.torikmTarget.focus();
	},
	computed: {
		// 一覧リストのカウントを計算する
	...
	methods: {
		...
		clickTorikmBtn: function() {
			...
			// ** チェック処理 **
			this.$refs.torikmTarget.validate();
			...
			// CSVファイルをアップロードする
			const self = this;
			co_postFile(PATH_PCSK010_READCSV, this.form, function(res) {
