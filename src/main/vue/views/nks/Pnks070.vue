	...
	<div>
		...
		<main id="PNKS070">
			<div class="contents__block--exist_return">
				...
				<h1 class="header__block-title">価格取込用データ出力</h1>
...
export default {
	...
	methods: {
		...
		clickCsvOutputBtn: function() {
			...
			const self = this;
			co_post(PATH_PNKS070_OUTPUT_CHECK, this.form, function(res) {
				...
				if (res.data > 0) {
					self.$store.commit(STORE_VIEW_PROGRESS, false);
					co_confirm(co_getMessage('NKS0002W'), function(btnValue){
		...
		csvOutput: function() {
			...
			// CSVファイルをダウンロードするリクエスト
			const self = this;
			co_downloadFile(PATH_PNKS070_OUTPUT, this.form, function(res, downloadHandle) {
