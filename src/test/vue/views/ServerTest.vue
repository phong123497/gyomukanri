...
(例)：
{
	...
	,keyword: ''
					...
					<div style="text-align: left;">
						<input type="button" class="contents__btn-blue" value="GET" v-on:click="clickGetBtn">&nbsp;
						<input type="button" class="contents__btn-blue" value="POST" v-on:click="clickPostBtn">&nbsp;
						<input type="button" class="contents__btn-blue" value="PUT" v-on:click="clickPutBtn">&nbsp;
						<input type="button" class="contents__btn-blue" value="DELETE" v-on:click="clickDeleteBtn">
					</div>
...
import { co_get, co_post, co_put, co_delete } from '@/common/ajaxUtility.js';
...
export default {
	...
	methods: {
		// GET送信ボタン押下時処理
		clickGetBtn: async function () {
			this.$store.commit(STORE_VIEW_PROGRESS, true);
			const json = await co_get(this.url, eval('(' + this.form + ')'), this.callback);
			if (json != null) {
		...
		clickPostBtn: async function () {
			this.$store.commit(STORE_VIEW_PROGRESS, true);
			const json = await co_post(this.url, eval('(' + this.form + ')'), this.callback);
			if (json != null) {
				this.entity = JSON.stringify(json, null, '\t');
			}
			this.entityDisplayFlg = true;
		...
		// PUT送信ボタン押下時処理
		clickPutBtn: async function () {
			this.$store.commit(STORE_VIEW_PROGRESS, true);
			const json = await co_put(this.url, eval('(' + this.form + ')'), this.callback);
			if (json != null) {
				this.entity = JSON.stringify(json, null, '\t');
			}
			this.entityDisplayFlg = true;
		...
		clickDeleteBtn: async function () {
			this.$store.commit(STORE_VIEW_PROGRESS, true);
			const json = await co_delete(this.url, eval('(' + this.form + ')'), this.callback);
			if (json != null) {
				this.entity = JSON.stringify(json, null, '\t');
			}
			this.entityDisplayFlg = true;
